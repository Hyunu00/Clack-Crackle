...
| **A4** | 엔진 순수 함수 (P6) 🟢 막힌 것 없음 | `02-ECONOMY` | `npm test` + 20시간 시뮬이 100ms 안 |
| **A5** | sim + balance 단언 (P7) | `02-ECONOMY` 감시 지표 4개 | `npm run sim` 이 지표 4개를 실제 숫자로 출력 |

**A1 을 먼저 끝낼 것.** B 가 여기 막혀 있다.

**A 레인을 막던 미결은 2026-09 에 전부 풀렸다.** Q1(자동 파괴 → 등급별
스킬트리 해금), Q11(연타 폐지로 소멸). 지금 A4 는 막힌 것이 없다.

A1 에서 챙길 것:
- **연타가 없다.** 진행률 0~1 하나를 꾹 누르기로만 올린다
- **끊어 누르기** 때문에 "꾹 시작 / 꾹 중단"이 이벤트여야 한다.
  떼도 진행률은 유지된다 — 상태에 남고 이벤트로만 알린다
- 파괴 이벤트에 방식(`hold`/`auto`)이 실려야 한다. 자동도 소리가 나는데
  볼륨이 달라서다
- **균열 단계 수는 설정값**(기본 4, 3~5)이다. 단계 수가 파괴 시간이나
  골드에 영향을 주면 안 된다
...
| **B3** | 왁뿌볼 렌더 (P8) 🟢 막힌 것 없음 | `03-ART` 직교 구조 | 종류×등급을 **런타임 조합**. 250텍스처가 아니다 |
| **B4** | HUD 뼈대 + 설정 화면 | `05-SHIP` 게임 내 필수 | 언어 목록이 `LOCALES` 를 읽어서 그려진다 |
| **B5** | 보관함 확대 화면 (P11) 🟡 방향만 확정 | `02-ECONOMY` §4 | 자동 파괴 해금 등급이면 안 뜬다 + "항상 이렇게" |
...
### Q3(화면 레이아웃) — 오너가 프로토타입 제작 중

**2026-09 기준 오너가 직접 만들고 있다.** B 는 그게 나오면 그걸 기준으로
잡는다. 논점은 키캡과 필드 사이 거리 — 수천 번 왕복할 동선이라
가깝게 붙이는 쪽을 권한다.

프로토타입이 늦어지면 B 가 `npm run shot` 으로 안을 2~3개 찍어 올려도 된다.
B3(왁뿌볼 렌더)와 B2(오디오)는 레이아웃과 무관하니 먼저 진행할 것.
...
미결 항목에 걸렸다
   ...
   │
   └─ 그 외             → 진행하지 말고 오너에게 묻는다.
                          답이 나오면 01-OPEN-QUESTIONS → 06-DECISIONS 로 옮긴다
```

**지금 열려서 코드를 막고 있는 것 (2026-09 갱신):**

| | 무엇 | 막는 레인 |
|---|---|---|
| Q5 | 보관함 확대 화면의 **세부** (클릭 수 등) | B5 — 방향은 확정, 일단 만들면 된다 |
| Q3 | 화면 레이아웃 | 오너가 프로토타입 제작 중 |

**코드를 완전히 막는 미결은 지금 없다.** 위 둘은 "일단 만들고 실플레이 후
조정" 이라 진행할 수 있다.

**2026-09 에 풀린 것:** Q1(자동 파괴 → 등급별 스킬트리 해금),
Q4(Supreme → 레인보우 색상 순환), Q11(연타 폐지로 소멸),
파괴 3초 + 끊어 누르기, 균열 단계 기본 4(설정 3~5).
전문은 `docs/06-DECISIONS.md`.
