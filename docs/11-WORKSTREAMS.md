...
| **A4** | 엔진 순수 함수 (P6) 🟡 **Q11 필요** | `02-ECONOMY` | `npm test` + 20시간 시뮬이 100ms 안 |
| **A5** | sim + balance 단언 (P7) | `02-ECONOMY` 감시 지표 4개 | `npm run sim` 이 지표 4개를 실제 숫자로 출력 |

**A1 을 먼저 끝낼 것.** B 가 여기 막혀 있다.

**Q1(자동 파괴 판정 단위)은 2026-09 에 풀렸다** — 등급별 스킬트리 해금이다.
A4 를 막던 큰 건은 사라졌고, 남은 건 Q11(연타와 꾹을 섞었을 때 골드 배율)
하나다. 작은 항목이라 A4 대부분은 그대로 진행할 수 있다.

A1 에서 챙길 게 늘었다 — **끊어 누르기** 때문에 "꾹 시작 / 꾹 중단"이
이벤트여야 하고, 파괴 이벤트에 방식(`tap`/`hold`/`auto`)이 실려야 한다.
자동 파괴도 소리가 나는데 볼륨이 달라서다.
